...
class CapsuleNet(nn.Module):
    ...
    def forward(self, x, y=None):
        x = F.relu(self.conv1(x), inplace=True) #x shape [batch,256 20 ,20
        x = self.primary_capsules(x)
        x = self.digit_capsules(x).squeeze().transpose(0, 1) #转置
        ...
        reconstructions = self.decoder((x * y[:, :, None]).view(x.size(0), -1))#解码 shape[batch ,28*28]
